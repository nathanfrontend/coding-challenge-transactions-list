...
const SendTransaction: React.FC<WalletProp> = ({ wallet }) => {
  //Implimented form state for validation purposes
  ...
  //reset form on close using event listener
  window.addEventListener("close.hs.overlay", ($overlayEl) => {
    reset({ to: "", amount: "" });
  });

  //adding payload to action to be recieved in saga
            ...
            <div className="bg-white border shadow-sm rounded-xl w-modal">
                  ...
                  {...register("to", {
                    ...
                    validate: (fieldValue) => {
                      return (
                        ...
                        "Wallet address must be 42 chracters"
                      );
                    },
                  })}
                />
                {errors.to && (
                  <p className="text-red-500 mt-2 "> {errors.to?.message}</p>
                )}
                <label
              ...
              <div className="flex justify-end items-center gap-x-2 py-3 px-4 border-t">
                ...
                <button
                  ...
                  // data-hs-overlay="#hs-basic-modal"
                  className=" hs-dropdown-toggle py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm"
                >
                  Send
                </button>
              </div>
